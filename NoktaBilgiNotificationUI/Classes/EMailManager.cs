    ...
    internal class EMailManager
    {
     ...
     string ficheLogicalRef)
        ...
        WHERE NKT.ID = {ficheLogicalRef}", dt.Rows[0][0].ToString());
            ...
            {
                XtraMessageBox.Show("Web Ayarları Bilgilerinizi Doldurunuz", "Hatalı Veritabanı İşlemi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            ...
            {
                XtraMessageBox.Show("Hatalı Web Sunucu MailAndWpCount Okuma İşlemi", "Hatalı Okuma", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            string tokenValue = $"{linkData.Rows[0]["WebToken"]}|{linkData.Rows[0]["WebPassword"]}|{fiche.Rows[0]["FICHENO"]}|{fiche.Rows[0]["FISLOGICAL"]}";
...
<table style='font-family: Arial, sans-serif; font-size: 14px;'>
    <tr>
        ...
        <td style='vertical-align: top;'>
            ...
            string subject = $"{fiche.Rows[0]["FICHENO"]} Nolu LOGO Sipariş Onay Bekleniyor.";
            string body = $@"
<html>
<body>
    <p>Sn. <strong>{manager}</strong>, <strong>{Convert.ToDateTime(fiche.Rows[0]["FICHEDATE"]).ToString("dd.MM.yyyy")}</strong> tarihli 
    <strong>{fiche.Rows[0]["USERCREATED"]}</strong> kullanıcısı tarafından 
    <strong><i>{fiche.Rows[0]["FICHENO"]}</i></strong> nolu, 
