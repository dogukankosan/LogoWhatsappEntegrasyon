    ...
    internal class PDFCreat
    {
        internal static byte[] PdfCreater(string id)
        {
            try
            {
                ...
                FROM LG_{dt.Rows[0][0]}_{dt.Rows[0][1]}_ORFICHE FICHE WITH (NOLOCK)
                JOIN LG_{dt.Rows[0][0]}_CLCARD CL WITH (NOLOCK) ON CL.LOGICALREF=FICHE.CLIENTREF
                LEFT JOIN L_CURRENCYLIST CUR WITH (NOLOCK) ON CUR.CURTYPE=FICHE.TRCURR AND FIRMNR={dt.Rows[0]["CompanyNo"].ToString()}
                LEFT JOIN LG_{dt.Rows[0][0]}_PAYPLANS PAY WITH (NOLOCK) ON PAY.LOGICALREF=FICHE.PAYDEFREF
                ...
                if (FicheOrder is null || FicheOrder.Rows.Count <= 0 || string.IsNullOrEmpty(FicheOrder.Rows[0]["Cari Açıklama"].ToString()))
                    return null;
...
ORDER BY LINE.LOGICALREF";
                ...
                AddInfoRow("Döviz Kuru:", $"{FicheOrder.Rows[0]["Kur"].ToString().Replace("$"," TL").Replace("€"," TL").Replace("£"," TL")}");
